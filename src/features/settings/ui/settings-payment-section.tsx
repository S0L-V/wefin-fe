...
function SettingsPaymentSection({ isLoggedIn }: SettingsPaymentSectionProps) {
  return (
    <div className="space-y-10">
      <section>
        <h3 className="mb-4 text-base font-semibold text-wefin-text">결제 수단</h3>
        <div className="flex items-center justify-between gap-4 rounded-xl border border-wefin-line bg-wefin-bg p-5">
          <div className="flex items-center gap-4">
            <div className="flex h-8 w-12 items-center justify-center rounded border border-wefin-line bg-white shadow-sm">
              <span className="text-[9px] font-bold italic text-blue-600">VISA</span>
            </div>
            <div>
          ...
          <button
            type="button"
            disabled={!isLoggedIn}
            className="h-8 rounded-lg border border-wefin-line px-3 text-xs font-semibold text-wefin-text transition-colors hover:bg-white disabled:cursor-not-allowed disabled:opacity-50"
          >
            변경
          </button>
        </div>
      ...
      <section>
        <h3 className="mb-4 text-base font-semibold text-wefin-text">결제 내역</h3>
          ...
          {paymentHistory.map((h) => (
            <div key={h.id} className="flex items-center justify-between py-4">
              <div className="space-y-0.5">
                <p className="text-[11px] font-medium uppercase tracking-wider text-wefin-subtle tabular-nums">
                  {h.date}
                </p>
                <p className="text-sm font-semibold text-wefin-text">{h.plan}</p>
              </div>
              <div className="space-y-0.5 text-right">
                <p className="text-sm font-bold text-wefin-text tabular-nums">₩ {h.amount}</p>
                <p className="text-[10px] font-semibold text-wefin-mint">{h.status}</p>
              </div>
            </div>
          ))}
        </div>
        <div className="mt-3 flex justify-center">
          <button type="button" className="text-xs text-wefin-subtle hover:text-wefin-text">
            더 보기
          </button>
        </div>
      </section>
